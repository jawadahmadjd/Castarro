...
def assert_owner_oauth_seed_refreshes_existing_owner_credentials() -> None:
    original_legacy_root = os.environ.get("STREAM_LEGACY_ROOT")
        ...
        try:
            ...
            config = {
                "youtube": {
                    ...
                    "use_pkce": False,
                    "accounts": [{"id": "account-1", "label": "Viewer channel"}],
                    "default_privacy_status": "public",
                }
            }

            assert runtime_paths.apply_youtube_owner_seed(config)
            assert config["youtube"]["client_id"] == "seed-client"
            assert config["youtube"]["client_secret"] == "existing-secret"
            assert config["youtube"]["oauth_client_type"] == "desktop"
            assert config["youtube"]["use_pkce"] is True
            assert config["youtube"]["default_privacy_status"] == "public"
            assert config["youtube"]["accounts"][0]["id"] == "account-1"
        finally:
            if original_legacy_root is None:
...
if __name__ == "__main__":
    ...
    assert_owner_oauth_seed_refreshes_existing_owner_credentials()
